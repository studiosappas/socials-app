...
-- rename only -- table/route/action names are unchanged). This adds an
-- optional folder layer above stories; a story row already models "one item
-- -> many files" via story_frames, so no new file-grouping table is needed,
-- just a place to group items.
...
drop policy if exists "Members can view content folders" on public.content_folders;
